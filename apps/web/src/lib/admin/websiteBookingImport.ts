...
function customerMatchesPhone(customer: ExistingCustomerOption, phone: string) {
  return normalizePhone(customer.phone) === phone || customer.additionalPhones.some((value) => normalizePhone(value) === phone);
}

function customerMatchesEmail(customer: ExistingCustomerOption, email: string) {
  const normalized = email.trim().toLowerCase();
  if (!normalized) return false;
  return customer.email === normalized || customer.additionalEmails.some((value) => value === normalized);
}

function customerLastNames(customer: ExistingCustomerOption) {
  const values = [customer.fullName, ...customer.additionalNames]
    .map((value) => normalizeName(value).split(" ").filter(Boolean).at(-1) ?? "")
    .filter(Boolean);
  return new Set(values);
}

function rowLastName(row: WebsiteBookingCsvRow) {
  return normalizeName(row.lastName || row.fullName.split(" ").filter(Boolean).at(-1) || "");
}

function narrowCustomersByEmail(customers: ExistingCustomerOption[], email: string) {
...
function narrowCustomersByLastName(customers: ExistingCustomerOption[], lastName: string) {
...
function matchCustomer(row: WebsiteBookingCsvRow, customers: ExistingCustomerOption[]) {
  ...
  if (byPhone.length > 1) {
    ...
    if (byPhoneThenEmail.length === 1) {
      return { customerId: byPhoneThenEmail[0].id, matchStatus: "matched" as const, reviewReason: "" };
    }

    const byPhoneThenLastName = narrowCustomersByLastName(byPhoneThenEmail, lastName);
    if (byPhoneThenLastName.length === 1) {
      return { customerId: byPhoneThenLastName[0].id, matchStatus: "matched" as const, reviewReason: "" };
    }
  ...
  if (byEmail.length === 1) {
    return { customerId: byEmail[0].id, matchStatus: "matched" as const, reviewReason: "" };
  }
  if (byEmail.length > 1) {
    const byEmailThenLastName = narrowCustomersByLastName(byEmail, lastName);
    if (byEmailThenLastName.length === 1) {
      return { customerId: byEmailThenLastName[0].id, matchStatus: "matched" as const, reviewReason: "" };
    }
  }
  ...
  if (byName.length > 1) {
    ...
    if (byNameThenLastName.length === 1) {
      return { customerId: byNameThenLastName[0].id, matchStatus: "matched" as const, reviewReason: "" };
    }
  ...
  if (byLastName.length === 1) {
    return { customerId: byLastName[0].id, matchStatus: "matched" as const, reviewReason: "" };
  }
  if (byLastName.length > 1) {
    return { customerId: "", matchStatus: "review" as const, reviewReason: `Multiple customers match last name ${row.lastName || row.fullName}.` };
  }

  return { customerId: "", matchStatus: "review" as const, reviewReason: `No customer matched phone ${row.phone}, email ${row.email}, or name ${row.fullName}.` };
}
